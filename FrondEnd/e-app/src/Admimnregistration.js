...
function Admimnregistration() {
    const nav=useNavigate()
    const [Adreg, setAdreg] = useState({})
    const handleChange=(event)=>{
const name=event.target.name
const value=event.target.value
setAdreg({...Adreg,[name]:value})
    }
    ...
    const handleButton=async()=>{
        try{
            ...
            const data=await axios.post("http://localhost:4000/adminsignup",Adreg,config)
            console.log(data)
            nav("/login")
            return data
...
<Col md='9' className='pe-5'>
  <input label='place' size='lg' id='form1' type='text'name='Password' onChange={handleChange}/>
</Col>
...
{/* 
        ...
        <hr className="mx-n3" />
<center>
        <Button className='buttadmin' onClick={handleButton}>Register</Button></center>
...
export default Admimnregistration